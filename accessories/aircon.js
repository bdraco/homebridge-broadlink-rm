...
class AirConAccessory extends BroadlinkRMAccessory {
  ...
  setDefaults () {
    ...
    config.defaultHeatTemperature = config.defaultHeatTemperature || 40;
  ...
  async sendTemperature (temperature, previousTemperature) {
    ...
    if (Array.isArray(hexData.data)) {
      for (let index = 0; index < hexData.data.length; index++) {
         await this.performSend(hexData.data[index]);
      }
    } else {
